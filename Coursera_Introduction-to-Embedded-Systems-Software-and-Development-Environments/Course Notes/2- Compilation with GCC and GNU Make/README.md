...
- The following image contains some important options to know:
...
- **These architecture flags are important to notify the compiler and the linker about specific libraries to include as well as the target architecture and CPU to cross-compile for.**
...
- We mentioned the term **compile time switch** before, let's discuss it in more details:
  ...
  - We may add the macro name to be defined from the command line itself without modifying the code. We do so by adding the option `-D<MACRO-NAME>` and please note that the <MACRO-NAME> comes immediately after the -D, this is equivalent to defining the macro in one of the source or header files.
...
- If we look at how the IDE performs the build process, we will find huge commands (+100 characters) and maybe (+10 flags) for each file type whether it's .c or .s or .o, etc.
...
- We can use GNU make to do more than just compile; we can use it to generate software dependancies, statistical information, etc.
